...
pull_images() {
    ...
    log_step "2/6" "Pulling CockroachDB images..."
...
start_containers() {
    ...
    log_step "3/6" "Starting $num_clusters CockroachDB containers..."
    for i in $(seq 0 $((num_clusters - 1))); do
        local version="${VERSIONS[$((i % ${#VERSIONS[@]}))]}"
...
wait_for_clusters() {
    ...
    log_step "4/6" "Waiting for clusters to be healthy..."
    for i in $(seq 0 $((num_clusters - 1))); do
        local sql_port=$((SQL_PORT_BASE + i))
        local http_port=$((HTTP_PORT_BASE + i))
        local name="${CONTAINER_PREFIX}-${i}"
...
generate_config() {
    ...
    log_step "5a/6" "Generating ${CONFIG_FILE}..."
...
# === Init ===
run_init() {
    log_step "5b/6" "Initializing history database on cluster-0..."
...
main() {
    ...
    case "$subcommand" in
        start)
            ...
            log_step "1/6" "Building Go binary..."
            go build -o "$BINARY" .
